...
<div class="container mx-auto px-4 py-8" data-aos="fade-up">
  ...
  <div class="bg-white shadow rounded-lg p-6 mt-4" data-aos="fade-up" data-aos-delay="100">
    ...
    @endif
    <form action="{{ route('admin.article.update', $article->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
      @csrf
      @method('PUT')
      <!-- Judul Artikel -->
      <div>
      ...
      <div>
        ...
        <select name="category" id="category" class="mt-1 block w-full border border-gray-300 rounded-md p-2 focus:outline-none focus:ring-2 focus:ring-masjid-blue">
          ...
          <option value="pesantren kilat" {{ old('category', $article->category) == 'pesantren kilat' ? 'selected' : '' }}>Pesantren Kilat</option>
          <option value="qurban" {{ old('category', $article->category) == 'qurban' ? 'selected' : '' }}>Qurban</option>
          <option value="kajian agama" {{ old('category', $article->category) == 'kajian agama' ? 'selected' : '' }}>Kajian Agama</option>
          <option value="bakti sosial" {{ old('category', $article->category) == 'bakti sosial' ? 'selected' : '' }}>Bakti Sosial</option>
        </select>
        @error('category')
          <span class="text-red-500 text-sm">{{ $message }}</span>
        @enderror
      ...
      <div>
        ...
        <div id="feature-dropzone" class="border-2 border-dashed border-gray-300 rounded-md p-6 flex flex-col items-center justify-center cursor-pointer transition hover:border-masjid-blue">
          ...
          <img id="feature-preview" src="{{ asset('storage/' . $article->image) }}" alt="" class="mt-4 max-h-60 rounded @if(!$article->image) hidden @endif">
        </div>
      ...
      <div>
        ...
        <div id="author-dropzone" class="border-2 border-dashed border-gray-300 rounded-full w-32 h-32 mx-auto flex items-center justify-center cursor-pointer transition hover:border-masjid-blue overflow-hidden">
          <img id="author-preview" src="{{ asset('storage/' . $article->author_photo) }}" alt="" class="w-full h-full object-cover @if(!$article->author_photo) hidden @endif">
          <span id="author-dropzone-text" class="text-gray-500" @if($article->author_photo) style="display: none;" @endif> Klik atau seret foto</span>
        </div>
